...
class SentinelSTACLoader:
    ...
    def search_images(self, bbox, start_date, end_date):
        ...
        try:
            ...
            iface.messageBar().pushMessage(
                "Error", "pystac-client library not found.", level=Qgis.MessageLevel.Critical)
            return []
        except Exception as e:
            iface.messageBar().pushMessage(
                "STAC error", str(e), level=Qgis.MessageLevel.Critical)
            return []
    ...
    def load_vrt(self, item, composition_name):
        ...
        try:
            ...
            iface.messageBar().pushMessage(
                "Error", "planetary-computer library not found.",
                level=Qgis.MessageLevel.Critical)
        except Exception as e:
            iface.messageBar().pushMessage(
                "VRT Error", str(e), level=Qgis.MessageLevel.Critical)
        return False
...
class SentinelSTACDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def popular_tabela(self):
        ...
        iface.messageBar().pushMessage(
            "Quick VRT Imagery Loader",
            f"{len(self.last_items)} images listed.",
